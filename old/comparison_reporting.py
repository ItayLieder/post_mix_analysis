...
def write_report_html(summary_df, deltas_df, plots, out_path, title, extra_notes=None):
    """
    Write the HTML report with tables and plots.
    Copies plots into the same directory as out_path unless already there.
    """
    report_dir = os.path.dirname(out_path)
    os.makedirs(report_dir, exist_ok=True)

    # Copy plots into the report directory if needed
    for k, p in plots.items():
        if os.path.exists(p):
            dst = os.path.join(report_dir, os.path.basename(p))
            # ✅ Fix: skip if source and destination are the same file
            if not os.path.samefile(p, dst):
                shutil.copy2(p, dst)
